<template>
  <Modal v-model="isShowModal" :title="`查看菜单功能—「${name}」`" width="950">
    <header>
      <div>
        <ButtonGroup>
...
export default class RemoveMenuFunction extends Mixins(PageMixins) {
  ...
  private deleteFun() {
    ...
    this.$Modal.confirm({
      ...
      content: `<p>您选择了<strong>${this.CurrentArray.length}</strong>条，确认要删除么？</p>`,
      onOk: () => {
        this.isLoading = true;
        this.mainManager.MenuService.BatchDeleteMenuFunctionAsync(
          ...
          if (res.success) {
            ...
            this.$Message.success(
              `成功删除了${this.CurrentArray.length}条功能`
            );
          }
          this.isLoading = false;
        });
