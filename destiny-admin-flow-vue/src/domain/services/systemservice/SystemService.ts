...
import {
  IAuditEntryOutputPageDto,
  IAuditEntryPropertyOutputPageDto,
...
export default class SystemService implements ISystemService {
  changePassword(_data: any): Promise<IAjaxResult> {
    return MainManager.dataRequest.postRequest(SystemApi.changePassword, _data);
  }
  getAuditPropertyList(_id: string): Promise<IAjaxResult> {
    return MainManager.dataRequest.getRequest(AuditApi.getAuditPropertyList, {
      id: _id,
    });
  }
  getAuditLogPage(
    _page: IPageRequest
  ): Promise<IServerPageReturn<IAuditLogOutputPageDto[]>> {
    return MainManager.dataRequest.postRequest(AuditApi.getAuditLogPage, _page);
  }

  getAuditEntryPage(
    _page: IPageRequest
  ): Promise<IServerPageReturn<IAuditEntryOutputPageDto[]>> {
    return MainManager.dataRequest.postRequest(
      AuditApi.getAuditEntryPage,
      _page
    );
  }

  getAuditEntryPropertyPage(
    _page: IPageRequest
  ): Promise<IServerPageReturn<IAuditEntryPropertyOutputPageDto[]>> {
    return MainManager.dataRequest.postRequest(
      AuditApi.getAuditEntryPropertyPage,
      _page
    );
  }
}
